...
motivation: 现有多模态流估计方法因缺乏统一的潜在表征空间，导致跨传感器特征匹配困难且融合系统过于复杂。
method: 引入以事件边缘为核心的同质空间，将图像和LiDAR特征与之对齐，并采用可靠性感知融合与跨维度对比学习优化预测。
result: 在多个合成与真实基准测试中均达到最先进水平，尤其在传感器退化的挑战性场景下改进显著。
conclusion: 通过构建统一的事件边缘空间，$x^2$-Fusion有效解决了多模态融合中的不一致性，提升了复杂动态场景下的运动估计精度。
---
