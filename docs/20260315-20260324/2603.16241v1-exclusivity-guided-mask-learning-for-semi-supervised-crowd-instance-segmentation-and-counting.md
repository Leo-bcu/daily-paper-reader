...
tldr: 针对半监督人群分析中点标注信息量不足的问题，本文提出了一种基于排他性引导的掩码学习框架。通过引入排除约束的双提示SAM（EDP-SAM）生成高质量掩码监督，并利用XMask模型强化空间分离。该方法将实例掩码先验作为伪标签，在多个主流数据集上实现了领先的分割与计数性能，有效统一了人群计数与实例分割任务。
selection_source: fresh_fetch
motivation: 传统的点标注难以捕捉细粒度的结构语义，限制了半监督人群分析在密集场景下的性能。
...
conclusion: 该研究通过引入丰富的实例掩码信息，成功弥合了人群计数与实例分割之间的鸿沟，提升了半监督学习的效率。
---

## 摘要
半监督人群分析是一个重要的研究领域，因为未标记的数据通常非常丰富且获取成本低廉。然而，传统的基于点的标注限制了性能，因为个体区域本质上具有歧义性，因此，从稀疏标注中学习细粒度的结构语义仍然是一个尚未解决的挑战。在本文中，我们首先提出了一种基于最近邻互斥圆（NNEC）约束的互斥约束双提示 SAM（EDP-SAM），用于为现有数据集生成掩码监督。针对密集场景中的个体分割，我们随后提出了互斥引导的掩码学习（XMask），该方法通过判别性掩码目标强制实现空间分离。利用高斯平滑和可微中心采样策略来提高特征连续性和训练稳定性。基于 XMask，我们提出了一个半监督人群计数框架，该框架使用实例掩码先验作为伪标签，其包含比传统点线索更丰富的形状信息。在 ShanghaiTech A、UCF-QNRF 和 JHU++ 数据集上（使用 5%、10% 和 40% 的标记数据）进行的广泛实验验证了我们的端到端模型实现了最先进的半监督分割和计数性能，有效地在一个统一框架内弥合了计数与实例分割之间的差距。
