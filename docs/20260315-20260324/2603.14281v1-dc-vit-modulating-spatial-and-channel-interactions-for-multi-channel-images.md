...
title_zh: DC-ViT：针对多通道图像的空间与通道交互调制
...
## 摘要
由于不同的染色方案、传感器类型和采集设置导致的多样化通道配置，多通道成像（MCI）中的训练和评估仍然具有挑战性。这种异质性限制了通用计算机视觉中常用的固定通道编码器的适用性。最近的多通道视觉 Transformer（MC-ViTs）通过支持灵活的通道输入来解决这一问题，通常是在统一的注意力空间内联合编码来自所有通道的 patch token。然而，跨通道的无限制 token 交互可能导致特征稀释，降低了保留 MCI 数据中至关重要的通道特定语义的能力。为此，我们提出了解耦视觉 Transformer（DC-ViT），它利用解耦自注意力（DSA）显式地调节信息共享。DSA 将 token 更新分解为两个互补的路径：建模通道内结构的空间更新，以及自适应整合跨通道信息的通道级更新。这种解耦在允许选择性跨通道交互的同时，缓解了信息崩溃。为了进一步利用这些增强的通道特定表示，我们引入了解耦聚合（DAG），使模型能够学习特定任务的通道重要性。在三个 MCI 基准测试上的广泛实验表明，该方法相比现有的 MC-ViT 方法具有持续的改进。
